...
def main(cfg):
    ...
    spectre         = build_spectral_net(cfg)
    chroma          = build_chroma_net(cfg)
...
def train_loop(
    ...
    rgbout = config.MODEL.OUTPUT_RGB
    ...
    while True:
        ...
        for datapoint in train_iterable:
            ...
            chroma_grad_norm_meter.update(chroma_grad_norm)

            # Checkpoint models
            if current_iter % config.LOGGING.CHECKPOINT_EVERY:
                pass


            # Log
            if current_iter % config.LOGGING.LOG_EVERY:
                ...
                logger.info(
                    f"Train: [{current_iter}/{max_iters}]\t"
...
if __name__ == "__main__":
    ...
    cfg = get_config("configs/testconfig.yaml")
    main(cfg)